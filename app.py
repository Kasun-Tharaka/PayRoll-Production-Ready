...
with selected_tabs[0]:
    ...
    if uploaded_file:
        ...
        # ACTIONS (Visible after calculation)
...
with selected_tabs[1]:
    ...
    with col_add:
        ...
        with st.form("add_emp_form"):
            ...
            new_nic = st.text_input("NIC No")
            new_bank = st.text_input("Bank Name")
            new_acc = st.text_input("Account No")
            new_date = st.text_input("Joined Date (YYYY-MM-DD)")
            if st.form_submit_button("Add to Database"):
                if new_id and new_name:
                    if add_employee(new_id, new_name, new_desig, new_dept, new_nic, new_bank, new_acc, new_date):
        ...
        if not df_emps.empty:
            ...
            if curr_data:
                ...
                with st.form("edit_emp_form"):
                    e_name = st.text_input("Name", value=c_name)
                    e_desig = st.text_input("Designation", value=c_desig)
                    e_dept = st.text_input("Department", value=c_dept)
                    e_nic = st.text_input("NIC", value=c_nic)
                    e_bank = st.text_input("Bank", value=c_bank)
                    e_acc = st.text_input("Account No", value=c_acc)
                    e_date = st.text_input("Joined Date", value=c_date)
                    
                    cb1, cb2 = st.columns(2)
                    if cb1.form_submit_button("Update"):
                        update_employee(c_id, e_name, e_desig, e_dept, e_nic, e_bank, e_acc, e_date)
                        st.rerun()
                    if cb2.form_submit_button("Delete", type="primary"):
                        delete_employee(c_id); st.rerun()
...
with selected_tabs[2]:
    ...
    if st.button("Retrieve Records"):
        ...
        if not hist_df.empty:
            ...
            st.metric("Total Payout", f"LKR {hist_df['net_salary'].sum():,.2f}")
        else:
