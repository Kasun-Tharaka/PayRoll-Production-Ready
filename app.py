...
if st.session_state['user_role'] == 'Admin':
    with selected_tabs[3]:
        ...
        if os.path.exists("system.log"):
            with open("system.log", "r") as log_file:
                log_content = log_file.read()
            st.download_button("📥 Download Full Log File", log_content, "system.log", "text/plain")
            with st.expander("View Recent Log Activity"):
                st.text_area("Log Output", log_content[-2000:], height=300)
        else:
            st.info("No logs generated yet.")

        # --- NEW: DATABASE DOWNLOAD BUTTON (ONLY CHANGE) ---
        st.divider()
        st.subheader("💾 Database Backup")
        st.write("Download a copy of the current database file for safe keeping.")
        
        if os.path.exists("property_payroll.db"):
            with open("property_payroll.db", "rb") as f:
                db_bytes = f.read()
            st.download_button(
                label="📥 Download Database Backup (.db)",
                data=db_bytes,
                file_name="property_payroll_backup.db",
